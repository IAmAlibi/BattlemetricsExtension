export async function getTeamLinks(BMToken, SteamID, YourServers) {
    ...
    try {
        ...
        if (resultText && !resultText.includes("Player not found")) {
            ...
            uniqueIDs = new Set(matches.filter(id => id !== SteamID));
        }
    } catch (err) {
        console.error(`Error fetching team info on server ${recentServerId}:`, err);
    }

    // Send batch update to API (include sender)
    try {
        const allIDsForUpdate = [...uniqueIDs, SteamID];
        fetch("http://51.222.17.247:25611/dict/update_team", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ steamids: allIDsForUpdate })
        }).catch(err => console.error("Failed to update teammates API:", err));
        // not awaiting, runs in background
    } catch (err) {
        console.error("Error sending update to API:", err);
    }

    // Resolve teammates info in parallel
    const bmPromises = [...uniqueIDs].map(async sid => {
        try {
            const res = await fetch(
            ...
            if (data?.data?.length > 0) {
                return { id: data.data[0].id, name: data.data[0].attributes.name || "" };
            }
        } catch (err) {
