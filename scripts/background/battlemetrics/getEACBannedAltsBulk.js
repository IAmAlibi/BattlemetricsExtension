...
export async function getEACBannedAltsBulk(BMToken, relatedPlayers) {
    ...
    for await (const relatedPlayer of relatedPlayers) {
        ...
        if (
            identifier.attributes.metadata !== null &&
            identifier.attributes.metadata.rustBans !== undefined &&
            identifier.attributes.metadata.rustBans.count > 0
        ) {
            bannedAlts.push({
                ...
                temp: identifier.attributes.metadata.rustBans.banned ? false : true,
            });
        }
    }
